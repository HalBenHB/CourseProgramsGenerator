...
# We no longer need to import list_programs here, as we are not re-generating the file
...
class StdoutRedirector:
    ...
    def write(self, string):
        ...
        if string.startswith('\r'):
            # Delete the content of the current last line
            self.text_widget.delete("end-1c linestart", "end")
            # Write the new progress update, stripping the \r
            # No newline is added, so the next normal print will start after it
            self.text_widget.insert("end", string.strip('\r'))
        else:
            # For all other output, just append it
    ...
    def flush(self):
        # This is called by print(..., flush=True), we can force an update
...
class App(tk.Tk):
    def __init__(self):
        ...
        self.requirements = []
        # --- NEW --- Store the path to the courses file for the caching mechanism
...
# Screen1 remains unchanged
class Screen1(ttk.Frame):
    def __init__(self, parent, controller):
        super().__init__(parent)
        self.controller = controller

        ttk.Label(self, text="Welcome to the Course Program Generator!", font=("Helvetica", 16)).pack(pady=20)
        ttk.Label(self, text="First, select the offered courses file (e.g., course_offered_2425F.xls).").pack(pady=10)

        file_frame = ttk.Frame(self)
        file_frame.pack(pady=10, fill='x', padx=20)
        self.courses_filepath_var = tk.StringVar(value=config.COURSES_FILEPATH)
    ...
    def browse_file(self):
        filename = filedialog.askopenfilename(initialdir=os.path.dirname(config.COURSES_FILEPATH), title="Select Courses File", filetypes=(("Excel Files", "*.xls*"), ("All files", "*.*")))
    ...
    def load_courses(self):
        ...
        try:
            # --- MODIFIED --- Store the chosen filepath in the controller
            self.controller.courses_filepath = filepath
            config.COURSES_FILEPATH = filepath

            all_courses = data_manager.course_parses()
...
class Screen2(ttk.Frame):
    def __init__(self, parent, controller):
        ...
        # --- NEW --- Configuration for predefined requirements
        self.PREDEFINED_REQS = {
            ...
            "BABUS Serbest Seçmeli": {
                "file": "BABUS Serbest Seçmeli.xls", # Assuming this file exists or will be created
                "needed": "<=1"
            },
        }
        ...
        # --- NEW --- The "Quick Add Requirement" panel
    ...
    # --- NEW --- Logic for the "Quick Add" panel
    def add_requirement_from_template(self):
        ...
        if selection == "Add from Custom File...":
            ...
            filepath = os.path.join(config.INPUT_FILEPATH, req_config['file'])
            req_name = selection
    ...
    # --- NEW --- Core logic for parsing file and creating the requirement
        ...
        try:
            ...
            for base_code in base_codes:
                ...
                for full_code in all_offered_courses:
                    # Check if 'ACCT 412.A' starts with 'ACCT 412.'
                    if full_code.startswith(base_code + '.'):
                        expanded_candidates.append(full_code)
                        found_match = True
                if not found_match:
                    print(f"Info: Base course '{base_code}' from the list had no offered sections this semester.")

            if not expanded_candidates:
                messagebox.showwarning("Warning", f"No offered courses found for the codes in '{os.path.basename(filepath)}'. An empty requirement was created.")
    ...
    def populate_editor_panel(self):
        ...
        # --- MODIFIED --- This block is now fixed and more robust
        ...
        # --- MODIFIED --- Use .get() for safety
        for candidate in sorted(req.get('candidates', [])):
            self.editor_candidates_listbox.insert(tk.END, candidate)
    ...
    def filter_courses(self):
        ...
        self.course_listbox.delete(0, tk.END)
        # --- MODIFIED --- Consistently use keys from the master dictionary
    ...
    def load_reqs(self, path=None):
        ...
        if path and os.path.exists(path):
            try:
                ...
                # Normalize the data: convert 'requirement_name' to 'name'
                ...
                # --- NEW --- Auto-select first item for better UX
    ...
    def load_last_session(self):
        temp_req_path = os.path.join(config.INPUT_FILEPATH, "requirements_gui_temp.json")
    ...
    def continue_to_screen3(self):
        ...
        config.requirements_parameter = "requirements_gui_temp.json"
...
class Screen3(ttk.Frame):
    def __init__(self, parent, controller):
        ...
        self.gen_vars = {
            "min_credit": tk.StringVar(value=config.min_credit),
            "max_credit": tk.StringVar(value=config.max_credit),
        ...
        self.out_vars = {"limit": tk.StringVar(value=config.limit_number_of_programs), "sort_str": tk.StringVar(value=config.sort_condition_str), "sort_reverse": tk.BooleanVar(value=False)}
        ...
        # --- NEW --- The "Sort Descending" checkbox
    ...
    # --- MODIFIED --- This function now generates a content-based hash for the filename
    def _generate_cache_filename(self):
        courses_path = self.controller.courses_filepath
        min_credit = self.gen_vars["min_credit"].get().strip() or "30"
        max_credit = self.gen_vars["max_credit"].get().strip() or "42"

        # Serialize requirements to a consistent string format
        req_string = json.dumps(self.controller.requirements, sort_keys=True, separators=(',', ':'))

        # Create a hash of the requirements string
        req_hash = hashlib.sha256(req_string.encode('utf-8')).hexdigest()[:16] # Use first 16 chars for brevity

        courses_basename = os.path.splitext(os.path.basename(courses_path))[0]
        safe_courses_name = "".join(c for c in courses_basename if c.isalnum() or c in (' ', '_')).rstrip()

        cache_filename = f"cache_{safe_courses_name}_reqs_{req_hash}_cr_{min_credit}-{max_credit}.pkl"
        return os.path.join(config.INPUT_FILEPATH, cache_filename)
    ...
    def save_output(self):
        ...
        # Suggest a filename based on the original auto-saved file
    ...
    def start_generation_thread(self):
        ...
        self.last_auto_save_path = None  # --- NEW --- Reset path at the start of each run
    ...
    def check_queue(self):
        try:
            ...
            self.after(100, self.check_queue)
             ...
             result_text = self.queue.get()
             print(result_text)
             self.generate_btn.config(state="normal", text="GENERATE PROGRAMS")

    def run_generation_worker(self):
        try:
            # --- MODIFIED --- Handle empty fields for credits and limit
            ...
            config.min_credit = int(min_credit_str) if min_credit_str else 30
            config.max_credit = int(max_credit_str) if max_credit_str else 42
            config.limit_number_of_programs = int(limit_str) if limit_str else None

            # --- MODIFIED --- Get the reverse sort state from the checkbox
            config.sort_reverse = self.out_vars["sort_reverse"].get()

            def parse_cs_string(s): return [item.strip() for item in s.split(',')] if s.strip() else None
            config.day_conditions = parse_cs_string(self.filter_vars["day_cond"].get())
            config.exclude_courses = parse_cs_string(self.filter_vars["exclude"].get())
            config.include_courses = parse_cs_string(self.filter_vars["include"].get())
            config.must_courses = parse_cs_string(self.filter_vars["must"].get())
            config.sort_condition_str = self.out_vars["sort_str"].get()

            # --- MODIFIED --- This is where the magic happens
            cache_filepath = self._generate_cache_filename()
            config.load_programs_if_saved = self.gen_vars["load_if_possible"].get()
            config.save_programs_after_generation = True
            config.save_output = True

            # Update config with the selected courses file from the controller
            config.COURSES_FILEPATH = self.controller.courses_filepath
            config.update_config()
            config.generation["programs_file_path"] = cache_filepath

            app_config = {"generation": config.generation, "output": config.output}

            print("Configuration updated. Starting generation process...\n")
            # --- MODIFIED --- Expect a three-part tuple back from the main logic
            summarized_programs, results_output, auto_save_path = run_program_generation(app_config)
